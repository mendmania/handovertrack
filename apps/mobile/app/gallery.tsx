...
function Gallery({ scope, projectId }: { scope: Scope; projectId?: string }) {
  const { captures, coordinator, identity, mediaWarning, retryUploads, reconcileCaptures } = useWorkspace();
  ...
  return <ScrollView style={styles.page} contentContainerStyle={styles.content}>
    ...
    <Text style={styles.text}>Originals and queue records stay on this device under your account. Uploads run while this app is open and connected. Originals remain on this device.</Text>
    <Button title={checking ? 'Checking local files…' : 'Check saved photos'} disabled={checking} onPress={() => void check()} />
    <Button title="Retry blocked uploads" onPress={() => void retryUploads().catch(() => {})} />
...
function Photo({ item }: { item: CaptureRecord }) {
  ...
  return <View style={styles.card}>
    ...
    <Text style={styles.small}>{imageError ? 'The preview failed. Check saved photos before relying on this original.' : saved && item.projectAvailable && item.queueState === 'pending' ? 'Queued for upload' : item.queueState === 'server_accepted' || item.queueState === 'completed' ? 'Uploaded · Server verified the original' : item.queueState === 'uploading' ? `Uploading · ${Math.round(item.bytesSent / (item.size || 1) * 100)}%` : item.queueState === 'failed' ? `Upload needs attention: ${item.queueReason}` : !item.projectAvailable ? 'Blocked: project access is unavailable. Remaining files and queue intent stay under your account.' : 'Queue needs attention. Remaining files and intent are kept; check saved photos to retry recovery.'}</Text>
