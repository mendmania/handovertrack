...
const env={...process.env,DATABASE_URL:process.env.TEST_DATABASE_URL,MIGRATION_DATABASE_URL:process.env.TEST_MIGRATION_DATABASE_URL};
if(!env.DATABASE_URL||new URL(env.DATABASE_URL).pathname!=='/handovertrack_test'||!['localhost','127.0.0.1'].includes(new URL(env.DATABASE_URL).hostname))throw Error('Isolated test DB required');
const config=readServerConfig(env),app=createApp(config,false),address=await app.listen({host:'127.0.0.1',port:0});
const admin=new pg.Pool({connectionString:env.MIGRATION_DATABASE_URL}),children:ChildProcess[]=[];
...
function start(){const child=spawn(process.execPath,['apps/worker/dist/main.js'],{env:{...env,WORKER_POLL_MS:'100',WORKER_LEASE_MS:'5000'},stdio:['ignore','pipe','pipe']});children.push(child);let output='';child.stdout!.on('data',b=>{output+=b.toString();});child.stderr!.on('data',b=>{output+=b.toString();});return {child,output:()=>output};}
try{
 const fixture=JSON.parse(await readFile(`${process.env.REPORT_TEST_OUTPUT??'.local/task07'}/fixture.json`,'utf8'));const {scope,projectId}=fixture;
 ...
 await writeFile(`${process.env.REPORT_TEST_OUTPUT??'.local/task07'}/worker-crash.json`,JSON.stringify({status:'PASS',reportId:report.id,...row},null,2));
