...
if(process.env.TASK07_ISOLATED!=='true'||!process.env.TEST_MIGRATION_DATABASE_URL)throw Error('Explicit isolated migration check required');
...
try{
 ...
 const before=await fingerprints();await migrate(url.toString());await migrate(url.toString());const after=await fingerprints();assert.deepEqual(after,before);await writeFile('.local/task07/migration-preservation.json',JSON.stringify({status:'PASS',tables:before},null,2));console.log(`PASS all ${tables.length} pre-existing tables unchanged by additive migration 006 and rerun`);
}finally{await c.end();}
