...
Kubernetes access and capacity checks pass. After owner sign-in, the authorized apex DNS-only A record was added for 159.195.30.113 and verified at both authoritative nameservers and two public resolvers. The workstation resolver still has a cached negative response, so its DNS preflight must be rerun after expiry. SSH now works as mendim using the netcup key with UseKeychain=yes; the prior failures omitted Keychain unlocking. The expired password-free sudo grant means OCI import still requires sudo authentication. A partial archive is retained privately on the VPS and must not be imported. GHCR package access still returns 403. Source `ba2ecc8d8040eda6e5d57bf73d6d17e90a4d2337` was committed and rebuilt from its exact Git archive. Its locally verified AMD64 image is `handovertrack.local/runtime@sha256:125cd8a42f0bb92bf87251d40c4cc14c561ab27e0f00911205e000856d66d7cc`; it is not imported/published. Release-controller maintenance tests and complete Caddy candidate validation pass. First-use certificate issuance correctly follows route creation.
